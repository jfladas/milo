...
function createDayPrlx() {
    const prlxDayData = [
        ...
        { id: "flwr3", src: "assets/flwr3.png", factor: 5, pos: [30] }
    ];
    prlxDayData.forEach(item => {
        const { id, src, factor, pos } = item;
        if (pos.length > 1) {
            pos.forEach(p => prlxDayItems.push(createPrlx(id, src, factor, p)));
        } else {
            prlxDayItems.push(createPrlx(id, src, factor, pos));
        }
    });
};
...
function createNightPrlx() {
    const prlxNightData = [
        ...
        { id: "flwr2", src: "assets/flwr2.png", factor: 5, pos: [100] },
        { id: "flwr1", src: "assets/flwr1.png", factor: 5, pos: [280] }
    ];
    prlxNightData.forEach(item => {
        const { id, src, factor, pos } = item;
        if (pos.length > 1) {
            pos.forEach(p => prlxNightItems.push(createPrlx(id, src, factor, p)));
        } else {
            prlxNightItems.push(createPrlx(id, src, factor, pos));
        }
    });
}